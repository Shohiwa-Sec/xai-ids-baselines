...
y = df.pop('ALERT')
X = df.drop(columns=['is_train'], errors='ignore')  # drop split-flag so it isn't used as a feature
...
# -----------------------------------------------------------------
# Preserve the original multiclass model and test set for Phase 2 SHAP.
# The AUC section below retrains/overwrites knn_clf and X_test.
# -----------------------------------------------------------------
_phase2_model = knn_clf
_phase2_X = X_test.copy()
...
# Use the original multiclass KNN saved before the AUC loop.
_phase2_X = _phase2_X.iloc[:min(500, len(_phase2_X))].copy()
# Defensive numeric cast: guards against any leaked non-numeric/bool
# column collapsing the frame to object dtype, which breaks SHAP's
# internal np.isclose/np.isfinite checks.
_phase2_X = _phase2_X.apply(pd.to_numeric, errors='coerce').fillna(0).astype(np.float64)
_phase2_background = _phase2_X.iloc[:min(50, len(_phase2_X))].copy()

# KNN is model-agnostic for SHAP, so explain predict_proba directly.
_phase2_explainer = shap.Explainer(_phase2_model.predict_proba, _phase2_background)
_phase2_shap = _phase2_explainer(_phase2_X)

# SHAP returns multiclass values as:
# samples x features x classes
_phase2_values = _phase2_shap.values if hasattr(_phase2_shap, 'values') else _phase2_shap
_phase2_values = np.asarray(_phase2_values)

if _phase2_values.ndim == 2:
    _phase2_values = _phase2_values[:, :, None]
elif _phase2_values.ndim != 3:
    raise ValueError(f'Unexpected SHAP value shape: {_phase2_values.shape}')

_phase2_n_samples = len(_phase2_X)
_phase2_n_features = _phase2_X.shape[1]
_phase2_class_names_raw = [str(x) for x in _phase2_model.classes_]
_phase2_n_classes = len(_phase2_class_names_raw)

# Normalize common SHAP multiclass layouts to:
# samples x features x classes
if _phase2_values.shape[0] != _phase2_n_samples:
    if (_phase2_values.shape[1] == _phase2_n_samples and
            _phase2_values.shape[2] == _phase2_n_features):
        _phase2_values = np.moveaxis(_phase2_values, 1, 0)
    elif (_phase2_values.shape[2] == _phase2_n_samples and
          _phase2_values.shape[1] == _phase2_n_features):
        _phase2_values = np.moveaxis(_phase2_values, 2, 0)

if (_phase2_values.shape[0] == _phase2_n_samples and
        _phase2_values.shape[1] != _phase2_n_features):
    if (_phase2_values.shape[2] == _phase2_n_features and
            _phase2_values.shape[1] == _phase2_n_classes):
        _phase2_values = np.moveaxis(_phase2_values, 1, 2)

if (_phase2_values.shape[-1] != _phase2_n_classes and
        _phase2_values.shape[0] == _phase2_n_classes):
    _phase2_values = np.moveaxis(_phase2_values, 0, -1)

if (_phase2_values.shape[0] != _phase2_n_samples or
        _phase2_values.shape[1] != _phase2_n_features):
    raise ValueError(
        f'Could not align SHAP values {_phase2_values.shape} '
        f'with X {_phase2_X.shape}'
    )

# Keep the actual label keys from the trained KNN, while displaying
# the normal class as "Normal" when it is stored as 0.
_phase2_label_map = {
    '0': 'Normal',
    '0.0': 'Normal',
    'Normal': 'Normal',
...
_phase2_classes = [
    _phase2_label_map.get(str(x), str(x))
    for x in _phase2_class_names_raw
]

# Mean absolute SHAP importance for each feature and class.
_phase2_mean_abs_shap = np.mean(np.abs(_phase2_values), axis=0)  # features x classes
_phase2_total_importance = _phase2_mean_abs_shap.sum(axis=1)

# Same top-feature approach as the RF SHAP plot.
_phase2_top_n = min(15, _phase2_n_features)
_phase2_top_indices = np.argsort(_phase2_total_importance)[::-1][:_phase2_top_n]

_phase2_plot_values = _phase2_mean_abs_shap[_phase2_top_indices]
_phase2_plot_features = [
    list(_phase2_X.columns)[i] for i in _phase2_top_indices
]

# Reverse so the largest feature is at the top of the horizontal chart.
_phase2_plot_values = _phase2_plot_values[::-1]
_phase2_plot_features = _phase2_plot_features[::-1]

fig, ax = plt.subplots(figsize=(9, 7))
_phase2_left = np.zeros(len(_phase2_plot_features))
_phase2_class_count = _phase2_plot_values.shape[1]
_phase2_colors = plt.cm.tab10(np.linspace(0, 1, _phase2_class_count))

for _phase2_class_idx in range(_phase2_class_count):
    _phase2_vals = _phase2_plot_values[:, _phase2_class_idx]
    ax.barh(
        _phase2_plot_features,
        _phase2_vals,
        left=_phase2_left,
        label=str(_phase2_classes[_phase2_class_idx]),
        color=_phase2_colors[_phase2_class_idx]
    )
    _phase2_left += _phase2_vals

ax.set_xlabel('mean(|SHAP value|) (average impact on model output)')
ax.set_ylabel('')
ax.set_title('KNN - Global SHAP Feature Importance')
ax.legend(title='Class', loc='lower right', fontsize=8)
fig.tight_layout()

fig.savefig(
    'KNN_Shap_Global_Feature_Importance.png',
    dpi=300,
    bbox_inches='tight'
)
plt.close(fig)

# Save the per-class SHAP importance table.
_phase2_shap_table = pd.DataFrame(
    _phase2_plot_values,
    columns=[str(x) for x in _phase2_classes],
    index=_phase2_plot_features
)
_phase2_shap_table.index.name = 'Feature'
_phase2_shap_table['Total mean(|SHAP|)'] = _phase2_shap_table.sum(axis=1)
_phase2_shap_table.to_csv('KNN_Shap_Global_Feature_Importance.csv')

print('Phase 2 SHAP figure saved: KNN_Shap_Global_Feature_Importance.png')
print('Phase 2 SHAP values saved: KNN_Shap_Global_Feature_Importance.csv')
# -----------------------------------------------------------------