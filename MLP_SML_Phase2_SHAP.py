...
def oversample(X_train, y_train):
    ...
    # Guard against mixed int/str label dtypes (e.g. NaN alerts filled with 0
    # mixed with attack-name strings) - sklearn can't classify those as a target type.
...
#Fill NaN with 0s (keep ALERT as strings so it doesn't end up mixing
# int 0 with the attack-name strings, which breaks classification later)
alert_col = df.pop('ALERT')
df = df.fillna(0)
df = df.assign(ALERT = alert_col.fillna('None'))
...
# Snapshot the trained multiclass model, its test set, predictions and class
# labels before the AUCROC block below reuses `MLP`/`X_test`/`y_test`/`y_pred`
# (it retrains 3 one-vs-rest binary models in a loop, so those names end up
# holding the LAST binary sub-problem's 2-column data by the time later code
# runs). Both the SHAP section and the final "AUC_ROC total" calculation
# further down use these snapshots so they still refer to the real multiclass
# model instead of the leftover binary one from the loop.
_shap_model = MLP
_shap_X_test = X_test
_mc_y_test = y_test
_mc_y_pred = y_pred
_mc_classes = MLP.classes_
...
# Use the snapshotted multiclass y_test/y_pred/classes (see note above) -
# `y_test`/`y_pred` here would otherwise still hold the AUCROC loop's last
# binary sub-problem, and its class labels are strings, not [0,1,2].
y_test_bin = label_binarize(_mc_y_test, classes=_mc_classes)
n_classes = y_test_bin.shape[1]
print('AUC_ROC total: ',AUC_ROC(y_test_bin,_mc_y_pred))
...
def _phase2_plot_global_shap(raw_values, X_explain, class_names, output_png, output_csv, title):
    ...
    class_importance = np.mean(np.abs(values), axis=0)  # features x classes
...
print('---------------------------------------------------------------------------------')
_phase2_X = _shap_X_test
_phase2_X = _phase2_X.iloc[:min(500, len(_phase2_X))].copy()
_phase2_background = _phase2_X.iloc[:min(50, len(_phase2_X))].copy()
_phase2_explainer = shap.Explainer(_shap_model.predict_proba, _phase2_background)
_phase2_shap = _phase2_explainer(_phase2_X)
_phase2_classes = [str(x) for x in _shap_model.classes_]
_phase2_plot_global_shap(
    _phase2_shap,
    _phase2_X,
    _phase2_classes,
    'MLP_Shap_Global_Feature_Importance.png',
    'MLP_Shap_Global_Feature_Importance.csv',
    'MLP - Global SHAP Feature Importance'
)