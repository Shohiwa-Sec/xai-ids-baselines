...
for j in range(0,len(y_array)):
    # print(j)
    #------------------------------------------------------------------------------------------------------------
    X_train,X_test, y_train, y_test = sklearn.model_selection.train_test_split(X, y_array[j].values.ravel(), train_size=split)
...
for i in range(0,len(TP)):
    Acc = ACC(TP[i],TN.iloc[i], FP[i], FN[i])
    print('Accuracy: ', label[i] ,' - ' , Acc)

# Use the original multiclass test labels and predictions for the final AUC.
# The AUC loop above overwrites y_test and y_pred with binary-model outputs.
label_to_number = {label[i]: i for i in range(len(label))}
y_test_multiclass_number = pd.Series(y_test_multiclass).map(label_to_number).to_numpy()
y_test_bin = label_binarize(y_test_multiclass_number,classes = np.arange(len(label)))
n_classes = y_test_bin.shape[1]
print('AUC_ROC total: ',AUC_ROC(y_test_bin,y_pred_multiclass))
