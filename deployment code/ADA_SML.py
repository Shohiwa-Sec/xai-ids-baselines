...
# Keep the original DataFrame index so ALERT values stay aligned with df after sampling.
y = pd.Series(y, index=y_filled.index)
...
# Keep the SAME label encoding for actual and predicted values.
# y was already encoded once above with LabelEncoder, so factorizing
# test['ALERT'] and y_pred separately can assign different numeric IDs.
y_test = pd.Series(test['ALERT']).astype(int).to_numpy()
...
# Build the confusion matrix using the shared encoded class IDs.
all_unique_values = np.arange(len(label_names))
confusion_matrix = pd.crosstab(
    pd.Series(y_test),
    pd.Series(pred_label),
    rownames=['Actual ALERT'],
    colnames=['Predicted ALERT'],
    dropna=False
).reindex(index=all_unique_values, columns=all_unique_values, fill_value=0)
...
# TP is a NumPy array; TN/FP/FN are Pandas Series.
# Use positional indexing for the Series to avoid label-index errors.
for i in range(len(TP)):
    Acc = ACC(TP[i], TN.iloc[i], FP.iloc[i], FN.iloc[i])
    class_label = label_names[i] if i < len(label_names) else f"Class {i}"
