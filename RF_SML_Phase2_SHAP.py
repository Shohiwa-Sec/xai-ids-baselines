...
def oversample(X_train, y_train):
    sampler = RandomOverSampler(sampling_strategy='minority', random_state=0)
    # Convert to numpy and oversample
    x_np = X_train.to_numpy()
    y_np = y_train.astype(str).to_numpy().ravel()
    x_np, y_np = sampler.fit_resample(x_np, y_np)
...
train, test, labels_train, labels_test = sklearn.model_selection.train_test_split(
    X, y, train_size=0.70, stratify=y, random_state=0
)
df = X.assign( ALERT = y)
...
# Train on every original observation.  Class weighting counters the imbalance
# without producing many duplicate attack rows.
print('Using class-weighted training; no training rows are duplicated.')
# ## Training the model
...
# Shuffle test rows without changing the class mix.
test = test.assign(ALERT=labels_test.to_numpy()).sample(frac=1, random_state=0)
...
print(classification_report(labels_test, pred_label, digits=4, zero_division=0))
...
# This section only analyzes/explains the already-trained RF model.
# It does not change the RF training, features, predictions, or hyperparameters.
...
X_shap = test.iloc[start_index:end_index]

# Compute SHAP values for the same test data used for evaluation.
shap_values_raw = explainer.shap_values(X_shap)

# SHAP can return either a list (older versions) or a 3-D array (newer versions).
if isinstance(shap_values_raw, list):
    ...
    if shap_values_3d.ndim == 2:
        shap_values_3d = shap_values_3d[..., np.newaxis]
else:
    shap_values_3d = np.asarray(shap_values_raw)
    if shap_values_3d.ndim == 2:
        shap_values_3d = shap_values_3d[..., np.newaxis]

if shap_values_3d.ndim != 3:
    raise ValueError(f'Unexpected SHAP output shape: {shap_values_3d.shape}')

# Mean absolute SHAP value for every feature, separately for every class.
mean_abs_shap = np.mean(np.abs(shap_values_3d), axis=0)  # features x classes

# Rank features by their total importance across all classes.
total_importance = mean_abs_shap.sum(axis=1)
feature_names = list(X_shap.columns)
top_k_features = min(15, len(feature_names))
top_indices = np.argsort(total_importance)[::-1][:top_k_features]

plot_values = mean_abs_shap[top_indices]
plot_features = [feature_names[i] for i in top_indices]

# Use the RF class order so the legend matches the SHAP class order.
class_names = [str(c) for c in rf.classes_]
if len(class_names) != plot_values.shape[1]:
    class_names = [f'Class {i}' for i in range(plot_values.shape[1])]

# Create the stacked horizontal bar chart like the reference figure.
y_pos = np.arange(len(plot_features))
fig, ax = plt.subplots(figsize=(10, 8))
left = np.zeros(len(plot_features))

for class_idx in range(plot_values.shape[1]):
    values = plot_values[:, class_idx]
    ax.barh(
        y_pos,
        values,
        left=left,
...
ax.set_title('RF Global Feature Importance using SHAP')
ax.legend(loc='lower right', frameon=False)
plt.tight_layout()
plt.savefig('RF_Shap_Global_Feature_Importance.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close(fig)

# Also save the values used to make the figure so the Phase 2 result is reproducible.
shap_table = pd.DataFrame(plot_values, index=plot_features, columns=class_names)
shap_table['Total mean(|SHAP|)'] = shap_table.sum(axis=1)
shap_table.to_csv('RF_Shap_Global_Feature_Importance.csv')

print('Saved: RF_Shap_Global_Feature_Importance.png')
print('Saved: RF_Shap_Global_Feature_Importance.csv')
