...
# Normalize ALERT labels to one consistent type.
# SIMARGL can contain the normal class as integer 0 while attack
# classes are strings (e.g. 'Denial of Service' and 'Port Scanning').
# RandomOverSampler/NumPy cannot sort mixed int + str labels.
y = df.pop('ALERT').astype(str)
X = df
print('---------------------------------------------------------------------------------')
print('---------------------------------------------------------------------------------')
print('---------------------------------------------------------------------------------')
...
start = time.time()
# Use the already-fitted RBF mapping for the test set.
X_test_ = rbf_feature.transform(X_test)
...
# Per-class metrics: TP is a NumPy array, while TN/FP/FN are
# pandas Series. Use positional indexing to avoid label-index errors.
for i in range(len(TP)):
    Acc = ACC(TP[i], TN.iloc[i], FP.iloc[i], FN.iloc[i])
    class_label = confusion_matrix.index[i]
    print('Accuracy: ', class_label, ' - ', Acc)
# #----------------------------------------------------------------
