"""Run original LightGBM training/evaluation with an all-feature execution fix.

Explanation sections and two dangling optional feature-filter lines are skipped.
Model settings are unchanged. Saved inputs are NOT a raw-traffic pipeline.
"""
...
SOURCE = ROOT / 'Light_ALL_FINAL_original.py'
...
evaluation_end = source.index('# Use KernelExplainer for model agnostic', evaluation_start)
...
# Original optional SHAP feature list is commented out while its uses remain
# active. Retain all features for this run; preserve original source on disk.
feature_filter_lines = ['newdf = newdf[selected_features]',
                        'newdf_test = newdf_test[selected_features]']
for statement in feature_filter_lines:
    if prefix.splitlines().count(statement) != 1:
        raise RuntimeError('Feature-selection section changed; review required.')
    prefix = prefix.replace(statement, '# All-feature run: ' + statement)
ast.parse(prefix)

output = ROOT / 'results' / ('original_light_saved_' + datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
...
try:
    ...
    with (output / 'console_output.txt').open('w', encoding='utf-8') as log:
        with contextlib.redirect_stdout(Tee(sys.stdout, log)), contextlib.redirect_stderr(Tee(sys.stderr, log)):
            ...
            joblib.dump(namespace['multi_target_lgbm'], output / 'lightgbm.joblib')
            ...
            print(f'\nSaved LightGBM, processed inputs, predictions and output to:\n{output}', flush=True)
finally:
    os.chdir(previous_directory)

metadata = {
    'source': 'https://github.com/ogarreche/XAI_NIDS/blob/main/NSL-KDD/Light_ALL_FINAL.py',
    ...
    'training_and_preprocessing_changes': 'Skip two references to undefined selected_features; retain all features. All other original preprocessing and training retained.',
    'evaluated_model_parameters': namespace['multi_target_lgbm'].estimator.get_params(),
    ...
    'notes': [
        'Original LightGBM settings: LGBMClassifier(random_state=0), multioutput classification.',
        'Original LightGBM script does not time training and prediction.',
        'Original LightGBM preprocessing and default model settings are retained; no oversampling added.',
