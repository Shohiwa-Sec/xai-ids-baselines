...
_phase2_X_np = _phase2_X.to_numpy(dtype=np.float32)
_phase2_background_np = _phase2_background.to_numpy(dtype=np.float32)
try:
    _phase2_explainer = shap.GradientExplainer(model, _phase2_background_np)
    _phase2_shap = _phase2_explainer(_phase2_X_np)
except Exception:
    _phase2_explainer = shap.Explainer(model, _phase2_background_np)
    _phase2_shap = _phase2_explainer(_phase2_X_np)
_phase2_classes = [str(x) for x in y_Label[:int(model.output_shape[-1])]]
_phase2_plot_global_shap(
    _phase2_shap,
    _phase2_X,
    _phase2_classes,
    'DNN_Shap_Global_Feature_Importance.png',
    'DNN_Shap_Global_Feature_Importance.csv',
    'DNN - Global SHAP Feature Importance'
)
