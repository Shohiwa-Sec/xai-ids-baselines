...
y_original = y.copy()          # keep raw labels for later identification
le = LabelEncoder()
y = le.fit_transform(y.astype(str))
y = pd.Series(y)     
label_names = le.classes_      # index -> original name
...
notused, y_labels = pd.factorize(y_original)
...
_phase2_explainer = shap.TreeExplainer(model)
_phase2_shap = _phase2_explainer.shap_values(_phase2_X)
_phase2_classes = [str(label_names[int(x)]) for x in model.classes_]
_phase2_plot_global_shap(
    _phase2_shap,
    _phase2_X,
    _phase2_classes,
    'LightGBM_Shap_Global_Feature_Importance.png',
    'LightGBM_Shap_Global_Feature_Importance.csv',
    'LightGBM - Global SHAP Feature Importance'
)
