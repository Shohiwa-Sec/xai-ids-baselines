"""Run the original KNN training/evaluation unchanged and save its artifacts.
...
SOURCE = ROOT / 'KNN_ALL_FINAL_original.py'
source = SOURCE.read_text(encoding='utf-8-sig')
marker = 'Y_train = Y_train.flatten()'
evaluation_marker = '#pred_labels = np.argmax(y_pred, axis=1)'
...
evaluation_end = source.index('# In[28]:', evaluation_start)
...
output = ROOT / 'results' / ('original_knn_saved_' + datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
...
try:
    ...
    with (output / 'console_output.txt').open('w', encoding='utf-8') as log:
        with contextlib.redirect_stdout(Tee(sys.stdout, log)), contextlib.redirect_stderr(Tee(sys.stderr, log)):
            ...
            joblib.dump(namespace['multi_target_knn'], output / 'knn.joblib')
            ...
            print(f'\nSaved KNN, processed inputs, predictions and output to:\n{output}', flush=True)
finally:
    os.chdir(previous_directory)

metadata = {
    'source': 'https://github.com/ogarreche/XAI_NIDS/blob/main/NSL-KDD/KNN_ALL_FINAL.py',
    ...
    'evaluated_model_parameters': namespace['multi_target_knn'].estimator.get_params(),
    ...
    'notes': [
        'Original KNN settings: n_neighbors=3, multioutput classification.',
        'The second printed training time actually measures prediction in the original code.',
        'Original minority oversampling and all KNN settings are retained.',
